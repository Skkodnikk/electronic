...
function addReview($pdo, $userId, $review) {
    $query = "INSERT INTO reviews (user_ID, review, created_at) VALUES (:user_ID, :review, NOW())";
    $statement = $pdo->prepare($query);
...
function deleteReview($pdo, $reviewId) {
    ...
    $statement = $pdo->prepare($query);
    $statement->bindParam(':id', $reviewId, PDO::PARAM_INT);
    $statement->execute();
}
...
if ($reviews) {
    foreach ($reviews as $review) {
        ?>
        <div>
            <p><strong>Отзыв (ID: <?php echo htmlspecialchars($review['ID']); ?>):</strong></p>
            <p><?php echo htmlspecialchars($review['review']); ?></p>
            <form method="post">
                <input type="hidden" name="reviewId" value="<?php echo $review['ID']; ?>">
                <input type="hidden" name="action" value="deleteReview">
                <button type="submit" style="margin-left: 1400px","color: white;">Удалить отзыв</button>
            </form>
        </div>
        <?php
    }
